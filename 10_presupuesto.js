...
// Show budget bars in balance cat list
function getBudgetBars(cat, subcat, spent, yr, m){
  const bud=getBudgetForMonth(yr,m);
  const key=subcat?`${cat}:${subcat}`:cat;
  const limit=bud[key];
  if(!limit||key==='__total__') return '';
  const pct=Math.min((spent/limit)*100,100);
  const over=spent>limit;
  const color=over?'var(--danger)':catColor(cat);
  return `<div class="bud-progress-wrap" style="margin-top:4px"><div class="bud-progress-bar" style="width:${pct.toFixed(0)}%;background:${color}"></div></div>
    <div class="bud-label-row"><span style="color:${over?'var(--danger)':'var(--text3)'}">${fmt(spent)} / ${fmt(limit)}</span><span style="color:${over?'var(--danger)':'var(--text3)'}">${pct.toFixed(0)}%</span></div>`;
}
