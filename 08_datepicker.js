...
function resetHistFiltersToTodos(){
  ...
  // Restablecer indicador del filtro de método si existe
  const methodPanel=document.getElementById('method-filter-panel');
  if(methodPanel) methodPanel.classList.remove('vis');
...
// Formatea un monto con separador de miles, respetando la moneda (para notas)
function fmtCur(n, cur){
  const num=Math.abs(n).toLocaleString('es-MX',{minimumFractionDigits:2,maximumFractionDigits:2});
